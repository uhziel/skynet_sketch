...
read_size(uint8_t * buffer) {
	int r =  (int)buffer[0] | (int)buffer[1] << 8 | (int)buffer[2] << 16 | (int)buffer[3] << 24;
	return r + SIZEOF_LENGTH;
}
...
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	if (size < SIZEOF_LENGTH) {
		...
		write_uncomplete_tmp_header(uc, buffer, size);
		return;
	}
...
filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	...
	if (uc) {
		// fill uncomplete
		if (uc->read < 0) {
			int size_for_header = write_uncomplete_tmp_header(uc, buffer, size);
			buffer += size_for_header;
			size -= size_for_header;
			if (uc->read > -SIZEOF_LENGTH) {
				return 1;
			}
		...
		printf("lua-netpackx51.c size:%d pack_size:%d\n", size, pack_size);
		if (size < pack_size) {
			struct uncomplete * uc = save_uncomplete(L, fd);
			uc->read = size;
			uc->pack.size = pack_size;
...
lpack(lua_State *L) {
	...
	*/
	...
	lua_pushinteger(L, len + SIZEOF_LENGTH);
...
LUAMOD_API int
luaopen_netpackx51(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "pop", lpop },
		{ "pack", lpack },
